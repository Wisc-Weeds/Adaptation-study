...
data2 %>% 
  ...
  filter(!is.na(gender)) %>% 
  mutate_if(is_character, as.factor) %>% 
  filter(month == "June") -> june
```


```{r}
gam_doy1 <- gam(gender_A ~ 
                 s(doyh, by = crop) + crop + 
                 s(location, by = year, bs = "re"), 
               family = "binomial", method = "REML", 
               data = june)
summary(gam_doy1)
plogis(coef(gam_doy1)[1])



plot(gam_doy1, residuals = FALSE, pch = 1, ylim = c(0,1), cex = 1,
     trans = plogis,  
     seWithMean = TRUE,
     shade = TRUE, 
     shift = coef(gam_doy)[1],
     shade.col = "lightgreen")

mgcv::predict.gam(gam_doy1, type = "response", se.fit = TRUE)
```
...
#       line=list(color = c("#30123bFF"), alpha = 0.1)
  ...
  labs(x = "Day of year", y = " Probability (%)") +
  facet_grid(scales = "free") +
  scale_y_continuous(limits = c(0, 1)) +
  scale_color_viridis_d(option = "C") +
  scale_fill_viridis_d(option = "C", alpha = 0.05) +
  annotate("text", x = 205, y = 0.90, label = "Second cohort",
            size = 5, color = 1) +
  theme_test() 
```
...
#                  month +
               ...
               family = "binomial", method = "REML", 
               data = data4)
plot(gam2, residuals = FALSE, pch = 1, ylim = c(0,1), cex = 1,
     trans = plogis,  
     seWithMean = TRUE,
     shade = TRUE, 
     shift = coef(gam_doy)[1],
...
```{r}
gam_doy2 <- gam(gender_A ~ 
                 s(height, by = crop) + 
                  s(height, by = crop) + 
                  s(weight, by = crop) + 
                 s(location, by = year, bs = "re"), 
               family = "binomial", method = "REML", 
               data = data4 %>% filter(month == "July"))
summary(gam_doy2)
plogis(coef(gam_doy2)[2])



plot(gam_doy2, residuals = FALSE, pch = 1, ylim = c(0,1), cex = 1,
     trans = plogis,  
     seWithMean = TRUE,
     shade = TRUE, 
     shift = coef(gam_doy)[1],
...
mgcv::predict.gam(gam_doy2, type = "response", se.fit = TRUE)
```
