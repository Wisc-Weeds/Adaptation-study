...
visreg(model_logi, "doyh", scale="response", rug=2, gg=TRUE,
       ...
       points=list(size=1, col = 1, pch=1),
       ylim = c(0,1)) -> doy_f
```
```{r}
doy_f +
  ...
  geom_emoji(aes(x = 220, y = 0.05), emoji = "2642") -> doyf
...
weight_f +
  ...
  geom_emoji(aes(x = 105, y = 0.05), emoji = "2642") -> weightf
...
height_f +
  ...
  theme_test() +
  geom_emoji(aes(x = 105, y = 0.95), emoji = "2640") +
  geom_emoji(aes(x = 105, y = 0.05), emoji = "2642") -> heightf
...
ggsave("figures/Figure 4.png", width = 9, height = 3)
```
...
#  as.data.frame() %>% 
  ...
  rename(Term = factor,
         ...
         Lower = lower,
         Upper = upper) %>% 
  dplyr::select(Term, AME, SE, Lower, Upper, `Z-score`, `P-value`) -> table_a

write_csv(table_a, "../data/table_a.csv")
```
