...
f5 <- fg4 | fg5 | fg6 +
  plot_annotation(
  title = NULL) 
  ggsave("height.png", width = 9, height = 5, dpi = 300, f5)
```
...
f3 <- p1c | p2c +
  ...
  ggsave("Combined.png", width = 6, height = 5, dpi = 600, f3)
```
...
data1 %>% 
  filter(!is.na(weight)) %>% 
  group_by(location, month, crop, doyh) %>% 
  summarise(biomass = sum(weight))  %>% 
  ggplot(aes(x = biomass, y = crop)) +
  geom_density_ridges() +
  facet_grid(location ~ month)
```
