...
biom %>%
  group_by(month) %>% 
  nest(-month) %>% 
  mutate(model = map(data, ~ lmer(log(biomass) ~ crop + 
                                 (1|year/location), data = .x)),
...
result %>% 
  ...
  ggplot(aes(x = fct_reorder(crop, response), y = response,
             color = crop)) +
  geom_point(size = 2) +
  geom_jitter(data = biom %>% 
                rename(response = biomass) %>% 
                filter(month == "First transplanting"), 
              alpha = 0.04, width = 0.3) +
...
ggsave("figures/first.png", width = 4, height = 4)
```
...
ggsave("figures/second.png", width = 4, height = 4)
```
